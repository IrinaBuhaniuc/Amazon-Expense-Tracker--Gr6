...
def generate_report(name,phone_number, purchases):
    ...
    average_cost = sum(orders_costs)/len(orders_costs)
    start_date = min(orders_dates).strftime("%m/%d/%Y")
    end_date = max(orders_dates).strftime("%m/%d/%Y")

    print(f"name: {name}       password: {password}     Tel: {stars_phone_number}     Date:{today}")
    print("------------------------------------------------")
    ...
    print(f"cost: {most_expensive[1]:.2f}", end = "                   ")  
    print(f"cost:{least_expensive[1]:.2f}")
    print("")
